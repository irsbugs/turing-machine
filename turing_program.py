...
State Halt (H)
    ...
    Code it returned as items in a python dictionary.

    δ(init,0) = (init, 1, R)

    Code in similar manner to Turing document syntax, but is actually a
...
def change_to_lower_case_x(δ):
    """
    Ensure dictionary is only strings and all characters are lowercase.
    """
    trf = {}
    for key, item in δ.items():
        if key[1] == "c" or key[1] == "C":
            trf[(str(key[0]), str(key[1]))] = item
        else:
            trf[(str(key[0]).lower(), str(key[1]).lower())] =(
                    str(item[0]).lower(), str(item[1]).lower(), str(item[2]).lower())
    ...
    trf[s,a] = (r, d, s1)
    print(trf)
    for key, item in trf.items():
        print(key, item)



    code = """
    # State 0:  Move right to end of first block of data
    (0, 0) (0, 0, r) # Move right if zeros
    (0, 1) (1, 1, r) # Move right if ones
    (0, _) (1, _, r) # Move right if underscores, past end of first block of data

    # State 1: Move right to end of second block of data
    (1, 0) (1, 0, r,) # Move right if zeros. Stay at state 1
    (1, 1) (1, 1, r,) # Move right if ones. Stay at state 1.
    (1, _) (2, _, l,) # Move left if underscore. To be back on data. Goto state 2
    ...
    for line in code.splitlines():
        ...
        if line.lstrip().startswith("("):
            ...
            result = line.replace(')','(').split('(')[1::2]
            #print("(" + result[0] + ")", "(" + result[1] + ")")
            print(result) #, result_1, result_2)

            print("state:", result[0][0], "read:", result[0][1])
